...
function binaryToString(binary) {
  const string = binary.split`0b`;

  return string.map((el) => String.fromCharCode(parseInt(el, 2))).slice(1)
    .join``;
  //////////////////////////////////!SECTION
  //   return String.fromCharCode(...binary.split(/(?=0b)/));
}
...
console.log('Hello World!'.split``.map((el) => el.charCodeAt()));
console.log(
  'Hello World!'.split``
