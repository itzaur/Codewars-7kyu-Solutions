...
function testResult(array) {
  ...
  const average =
    Math.round((array.reduce((acc, el) => acc + el, 0) / array.length) * 1000) /
    1000;

  return array.reduce((acc, el) => {
    if (el >= 9) acc.h += 1;
    else if (el >= 7) acc.a += 1;
    else acc.l += 1;

    return acc;
  }, result).h === array.length
    ? [average, result, 'They did well']
    : [average, result];
}
//////////////////////////////////////////////////!SECTION
...
//   ((obj) =>
//     [
//       Math.round((array.reduce((pre, val) => pre + val) / array.length) * 1e3) /
//         1e3,
//       obj,
//     ].concat(obj.a || obj.l ? [] : `They did well`))(
//     array.reduce(
//       (pre, val) => (val > 8 ? pre.h++ : val > 6 ? pre.a++ : pre.l++, pre),
//       { h: 0, a: 0, l: 0 }
//     )
//   );
