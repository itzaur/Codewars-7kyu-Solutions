...
function countName(anArr, name) {
  const occurrences = anArr.reduce(
    (acc, el) => ({ ...acc, [el]: (acc[el] || 0) + 1 }),
    {}
  );

  return occurrences[name] || 0;
  ////////////////////////////////////!SECTION
  //   return anArr.filter((x) => x == name).length;
  ////////////////////////////////////!SECTION
  //   return anArr.reduce((pre, val) => pre + (val === name), 0);
}
...
console.log(countName(MAINLIST, 'Amy')); //3
console.log(countName(MAINLIST, 'Bob')); //2
