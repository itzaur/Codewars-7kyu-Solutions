...
function symDiff(s1, s2) {
  // return new Set(s1).symmetricDifference(new Set(s2));

  return new Set([
    ...[...s1].filter((el) => !s2.has(el)),
    ...[...s2].filter((el) => !s1.has(el)),
  ]);
  //////////////////////////////////////!SECTION
...
console.log(symDiff(A, A), new Set());

console.log(symDiff(A, B)); //C
console.log(new Set([...symDiff(B, A)].sort())); //C
console.log(symDiff(A, B) instanceof Set); //true
