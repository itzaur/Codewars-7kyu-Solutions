...
function stickyCalc(operation, val1, val2) {
  const sum = +(`${Math.round(val1)}` + `${val2}`);
  const options = {
    '+': sum + val2,
    '-': sum - val2,
    '*': sum * val2,
  ...
  return Math.round(options[operation]);
}
...
console.log(stickyCalc('+', 4.2, 7)); //(47) + 7 = 54
console.log(stickyCalc('+', 4.7, 7)); //(57) + 7 = 64
console.log(stickyCalc('/', 10, 7)); //(107) / 7 = 15  Not 15.2857...
