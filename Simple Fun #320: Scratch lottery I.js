...
function scratch(lottery) {
  return lottery.reduce((acc, el) => {
    const [animal1, animal2, animal3, prize] = el.split` `;

    return acc + (animal1 === animal2 && animal2 === animal3 ? +prize : 0);
  }, 0);
  //////////////////////////////////////////////!SECTION
  //   return lottery.reduce(
  //     (pre, val) => pre + /(\w+ )\1\1/.test(val) * val.replace(/\D/g, ``),
  //     0,
  //   );
}
