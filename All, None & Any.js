...
Array.prototype.all = function (p) {
  return this.filter((el) => p(el)).length === this.length;
};

Array.prototype.none = function (p) {
  return this.filter((el) => p(el)).length === 0;
};
...
console.log(![-1, 0, 2].all(isGreaterThanZero)); //true
console.log(![-1, 2, 3].none(isLessThanZero)); //true
