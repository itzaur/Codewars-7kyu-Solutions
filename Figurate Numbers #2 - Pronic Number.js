...
*/
function isPronic(n) {
  const discriminant = 1 - 4 * (n > 0 ? -n : n);
  const n1 = (-1 + Math.sqrt(discriminant)) / 2;
  const n2 = (-1 - Math.sqrt(discriminant)) / 2;

  return (
    Math.abs(Math.abs(n1) - Math.abs(n2)) === 1 &&
  ...
  //   return false;
  ////////////////////////////!SECTION
  //   return Number.isInteger((4 * n + 1) ** 0.5);
}
